...
def build_bad_bronze_record():
    event_id = f"evt_bad_card_auth_{uuid4().hex[:8]}"
    parsed_value = {
        "event_id": event_id,
        "transaction_id": f"txn_bad_{uuid4().hex[:8]}",
        "authorization_id": f"auth_bad_{uuid4().hex[:8]}",
        "customer_id": "cust_100001",
        "account_id": "acct_200001",
        "card_id": "card_300001",
        "event_time": datetime.now(UTC).isoformat(),
        "amount": "42.50",
        # Currency is intentionally invalid for Silver card rules.
        "currency": "EUR",
        "merchant_id": "mrc_bad_test",
        "merchant_name": "Invalid Currency Test Merchant",
...
def main():
    ...
    object_key = (
        ...
        f"bad-card-authorization-{uuid4().hex}.json"
    )
